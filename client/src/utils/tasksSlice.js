...
const tasksSlice = createSlice({
    ...
    reducers: {
        ...
        getTask: (state, action) => {
            ...
            const task = state.value.find((task) => task._id === taskId);
        ...
        updateTask: (state, action) => {
            ...
            const taskIndex = state.value.findIndex(task => task._id === updatedTask._id);
            state.value[taskIndex] = updatedTask;
        },
        editTaskStatus: (state, action) => {
            const { taskId, newStatus } = action.payload;
            const taskIndex = state.value.findIndex(task => task._id === taskId);

            if(taskIndex !== -1){
                state.value[taskIndex].status = newStatus;
            }
        },
        moveToTrash: (state, action) => {
            ...
            if(taskIndex !== -1){
                state.value[taskIndex].isTrashed = true;
            }
        },
        restoreFromTrash: (state, action) => {
            ...
            if(taskIndex !== -1){
                state.value[taskIndex].isTrashed = false;
            }
        },
        permanentDeleteTask: (state, action) => {
            const taskId = action.payload;
            state.value = state.value.filter((task) => task._id !== taskId);
        },
        addSubtask: (state, action) => {
            const { taskId, newSubTasks } = action.payload;
            const taskIndex = state.value.findIndex(task => task._id === taskId);
            if(taskIndex !== -1){
                state.value[taskIndex].subTasks = [...newSubTasks];
            }
        },
        ...
        editSubTaskStatus: (state, action) => {
            const {taskId, subTaskId, newStatus} = action.payload;
            const taskIndex = state.value.findIndex(task => task._id === taskId);
            if(taskIndex !== -1){
                const subTaskIndex = state.value[taskIndex].subTasks.findIndex(subTask => subTask._id === subTaskId);
                if(subTaskIndex !== -1)
                {
                    state.value[taskIndex].subTasks[subTaskIndex].isCompleted = newStatus === "completed";
                }
            }
        },
        addActivity: (state, action) => {
            const {taskId, updatedActivities} = action.payload;
            const taskIndex = state.value.findIndex(task => task._id === taskId);
            if(taskIndex !== -1){
                state.value[taskIndex].activities = [...updatedActivities];
            }
        },
        uploadAsset: (state, action) => {
            const {taskId, updatedAssets} = action.payload;
            const taskIndex = state.value.findIndex(task => task._id === taskId);
            if(taskIndex !== -1){
                state.value[taskIndex].assets = [...updatedAssets];
            }
        }
...
export const { setTasks, getTask, addNewTask, updateTask, editTaskStatus, moveToTrash, restoreFromTrash, permanentDeleteTask, addSubtask, deleteSubtask, addActivity, uploadAsset, editSubTaskStatus } = tasksSlice.actions;
