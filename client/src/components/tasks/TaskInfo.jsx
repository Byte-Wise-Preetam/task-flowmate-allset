...
activities: [],
assets:[], 
createdAt: "2024-12-01T23:48:22.560Z",
description: "Develop a dashboard to manage tasks, view statuses, and assign team members.",
isTrashed: false,
lastDate: "2024-12-03T00:00:00.000Z",
priority: "high",
status: "inqueue",
subTasks: [],
taskAdmin: {_id: "674ac297502103a9ff3ac112", firstName: "Shri Radhe", lastName: "Krishna"},
team: [{memberId: {_id: "674b590ef8c4df6962de22dd", firstName: "Sophia", lastName: "Davis"}, role:"Data Analyst", email:"sophia.davis@example.com"}],
title: "Build Task Management Dashboard",
updatedAt: "2024-12-01T23:48:22.560Z",
__v: 0,
_id: "674cf5c6c710469564baea89",
}
...
const dummyActivity = [
    {
        ...
        _createdAt: "2024-12-01T23:48:22.560+00:00"
...
const TaskInfo = () => {
    ...
    return(
        <div className="max-w-[1536px] px-4 sm:px-8 py-4 bg-gray-100 mx-auto">
            <div className="text-xl sm:text-2xl font-semibold">{task.title}</div>

            <div className="text-sm sm:text-base font-medium text-gray-600 mt-1 mb-4">{task.description}</div>
            
            <div className="w-full flex flex-row gap-y-5 sm:gap-y-0 gap-x-8 mx-auto flex-wrap my-4 mb-6">
                <button onClick={() => setSelected(0)} className={`flex flex-row items-center text-sm sm:text-base font-medium bg-white px-4 py-3 ${selected === 0 ? "text-blue-500 border-b-2 border-blue-500" : "text-gray-800"}`}>
                    <FaList className="w-4 h-4 mr-2"/>
                    <span>Task Details</span>
                </button>

                <button onClick={() => setSelected(1)} className={`flex flex-row items-center text-sm sm:text-base font-medium bg-white px-4 py-3 ${selected === 1 ? "text-blue-500 border-b-2 border-blue-500" : "text-gray-600"}`}>
                    <RxActivityLog className="w-4 h-4 mr-2"/>
                    <span>Activities/Timeline</span>
                </button>

                {
                    task.status === "inqueue" && <button onClick={() => handleEditStatus("active")} className=" bg-blue-600 text-white flex flex-row items-center gap-x-2 text-sm sm:text-base font-bold px-3 py-2.5 sm:py-1.5 rounded-[3px]"><FaPlay className="w-3 h-3"/> <span>Start Task</span></button>
                }

                {
                    (task.status === "active" && isUserTaskAdmin) && <button onClick={() => handleEditStatus("completed")} className=" bg-blue-600 text-white flex flex-row items-center gap-x-2 text-sm sm:text-base font-bold px-3 py-2.5 sm:py-1.5 rounded-[3px]"><MdFileDownloadDone className="w-6 h-6"/> <span>Mark as Done</span></button>
                }

                {
                    (task.status === "completed" && isUserTaskAdmin) && <button onClick={() => handleEditStatus("active")} className=" bg-blue-600 text-white flex flex-row items-center gap-x-2 text-sm sm:text-base font-bold px-3 py-2.5 sm:py-1.5 rounded-[3px]"><FaUndoAlt className="w-4 h-4"/> <span>Undo</span></button>
                }
            </div>

            {
                selected === 0 && <div className="bg-white p-4 flex flex-col sm:flex-row gap-x-10">
                    <div className="w-full sm:w-[50%]">
...

                        {
                            task?.team.length !== 0 && <div className="my-4">
                                <div className="text-gray-800 text-base font-semibold mb-4">TASK TEAM</div>
                                <div>
                                    {
                                        task?.team.map((member, index) => (
                                            <div className="flex flex-row items-center border-t py-3" key={member.memberId._id}>
...

                        {
                            task?.subTasks.length !== 0 && <div>
                                <div className="text-gray-800 text-base font-semibold mb-4">SUB-TASKS</div>
                                <div>
                                    {
                                        task?.subTasks.map((subTask, index) => (
                                            <div className="flex flex-col gap-y-2 xl:gap-y-0 xl:flex-row justify-between items-start border-t py-3" key={subTask._id}>
                                                <div className="flex flex-row items-center">
                                                    ...
                                                    <div>
                                                        <div className="text-gray-600 text-sm font-medium flex flex-row items-center justify-between">
                                                            <span>Last Date : {dateFormatter(subTask.lastDate)}</span>
                    ...
                    <div className="w-full mt-8 sm:mt-0 sm:w-[50%]">
                        <div className="flex flex-col">
                            <div className="text-base text-gray-800 font-semibold uppercase mb-2">Assets</div>
                            
                            <div className="px-4 my-1">
                                <div className="border-2 border-gray-300 border-dashed py-8 px-2 text-center  font-semibold text-gray-700 cursor-pointer" {...getRootProps()}>
                                    <input {...getInputProps()}/>
                            ...
                            <div className="px-4">
                                {
                                    task.assets.map((asset) => (
                                        <div className="flex flex-row items-center justify-between p-2 mb-2 border border-gray-300 text-gray-800 text-sm font-medium" key={asset._id}>
                                            <a href={asset.url} target="_blank" className="line-clamp-1 mr-4">{asset.filename}</a>
                                            <div className="flex flex-row items-center gap-x-3 ">
                                                {/* <button className=""><MdOutlineFileDownload className="w-6 h-6"/></button> */}
                                                <span>{Math.floor(asset.bytes/1024)}kb</span>

                                                {
                                                    (asset.by === loggedinUser._id || isUserTaskAdmin) && <button className="text-gray-500" onClick={(e) =>{e.stopPropagation(); handleDeleteAssetFromCloudinary(asset.public_id, asset.resource_type);}}><FaTrashAlt className="w-3.5 h-3.5"/></button>
                                                }
                                            </div>
                                        </div>
                                    ))
